...
function createUserData(bixbyUserId, userData) {
  ...
  body.fields[properties.get('config', 'userDataField')] =
  ...
  if (response) {
    userData = JSON.parse(
      response.fields[properties.get('config', 'userDataField')]
    );
...
function getUserData(bixbyUserId) {
  ...
  const query = {
    filterByFormula:
      '{' + properties.get('config', 'userIdField') + '}="' + bixbyUserId + '"',
  };
  const options = {
    format: 'json',
    query: query,
  ...
  if (response && response.records && response.records.length === 1) {
    const userData = JSON.parse(
      response.records[0].fields[properties.get('config', 'userDataField')]
    );
